...
function normalizeGalleryCategory(value = "") {
  ...
  if (text.includes("sangeeth") || text.includes("sangeet")) return "sangeeth";
  if (text.includes("wedding") || text.includes("reception")) return "wedding";
  if (text.includes("vacation")) return "vacation";
  if (text.includes("guest") || text.includes("upload") || text.includes("family")) return "guest";
  return text || "guest";
}
...
function renderApprovedGalleryUploads() {
  ...
  const html = galleryRows.map(row => {
    ...
    const rawImageUrl = row.DirectFileUrl || row.ImageUrl || row.ImageURL || row.DriveLink || "";
    const imageUrl = toEmbeddableDriveImageUrl(rawImageUrl);
      ...
      return `
        <article class="gallery-slide dynamic-gallery-slide" data-category="${escapeAttr(category)}">
          ...
          <div class="gallery-caption">
            ...
            ${uploadedBy ? `<small>Uploaded by ${escapeHtml(uploadedBy)}</small>` : ""}
          </div>
        </article>
      `;
    }
...
function applyGalleryFilter(filter = "all") {
  ...
  const slides = Array.from(track.querySelectorAll(".gallery-slide:not(#galleryEmptyState)"));
  let visibleCount = 0;
  ...
  let empty = document.getElementById("galleryEmptyState");
  if (!empty) {
...
function initGallery() {
  ...
  tabs.forEach(tab => {
    ...
    tab.addEventListener("click", () => {
      ...
      applyGalleryFilter(tab.dataset.galleryFilter);
    });
  });

  if ($(".gallery-next")?.dataset.bound !== "true") {
    $(".gallery-next").dataset.bound = "true";
    $(".gallery-next")?.addEventListener("click", () => {
      track.scrollBy({ left: 380, behavior: "smooth" });
    });
  }

  if ($(".gallery-prev")?.dataset.bound !== "true") {
    $(".gallery-prev").dataset.bound = "true";
